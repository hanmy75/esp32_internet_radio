...
void app_main(void)
{
    ...
    audio_element_handle_t http_stream_reader, aac_decoder, bt_stream_writer;
    ...
    ESP_LOGI(TAG, "[3] Create aac decoder to decode aac file");
    ...
    ESP_LOGI(TAG, "[4.2] Link it together http_stream-->aac_decoder-->bt_stream_writer");
    const char *link_tag[3] = {"http", "aac", "bt"};
    audio_pipeline_link(pipeline, &link_tag[0], 3);

    ESP_LOGI(TAG, "[4.3] Set up  uri (http as http_stream, aac as aac decoder, and default output is bt)");
